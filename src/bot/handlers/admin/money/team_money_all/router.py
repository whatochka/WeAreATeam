...
@router.message(Command("team_money_all"))
async def admin_update_money(
    ...
    if command.args and len(command.args.split()) == 1:
        ...
        await users_service.update_team_balance_all(user.id, amount)
        await message.answer(f"Добавлено {amount} всем пользователям")
    else:
        await message.answer("Формат: /team_money_all <amount>", parse_mode=None)
