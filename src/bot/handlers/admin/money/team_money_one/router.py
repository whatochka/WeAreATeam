...
@router.message(Command("team_money"))
async def admin_update_money(
    ...
    if command.args and len(command.args.split()) == 2:
        ...
        await users_service.update_team_balance(slave_id, user_id, amount)
        await message.answer(f"Добавлено {amount} пользователю {slave_id}")
    else:
        await message.answer("Формат: /team_money <user_id> <amount>", parse_mode=None)
