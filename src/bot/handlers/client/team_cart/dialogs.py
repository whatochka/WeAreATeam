...
from .states import TeamCartStates

cart_window = Window(
    Multi(
        Format("🧺 <b>Корзина</b>"),
        Format("📦 <b>Количество товаров:</b> {total_products} шт."),
        Format("🛒 <b>Cписок покупок:</b>\n{formated_info}"),
        sep="\n\n",
        when=F["total_purchases"],
    ),
    Const(
        "📦 Корзина отряда пустая",
        when=~F["total_purchases"],
    ),
    ScrollingGroup(
        Multiselect(
...
refund_cart_window = Window(
    ...
    GoToMenuButton(),
    state=TeamCartStates.refund,
)

team_cart_dialog = Dialog(
    cart_window,
    refund_cart_window,
)
